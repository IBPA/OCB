...
class UnsupervisedValidation(v_module_template.ValidationSubModule):
    ...
    def validate_data(self):
        ...
        unsupervised_plot = results.plot(
            title='Unsupervised validation results (average of ' + str(self.parameters.n_trial) + ' trials)')
        unsupervised_plot.set(
            xlabel='noise ratio', ylabel='Symmetric mean absolute percentage error(%)')
        ...
        print('Benchmark: the area between error curve and reference (100% Error for all noise ratio) is ' +
              str(np.round(benchmark, 2)) + '(%)')

    def check_noise_ratio(self):
        self.parameters.noise_ratio = list(
            np.round(self.parameters.noise_ratio, 2))

        if max(self.parameters.noise_ratio) > 1 or min(self.parameters.noise_ratio) < 0:
            raise Exception(
                'The noise ratio for unsupervised validation should be a number of sequence and each number should be in [0,1]')
    ...
    def check_missing_value_ratio(self):
        self.parameters.missing_value_ratio = list(
            np.round(self.parameters.missing_value_ratio, 2))

        if max(self.parameters.missing_value_ratio) >= 1 or min(self.parameters.missing_value_ratio) < 0:
            raise Exception(
                'The missing value ratio for unsupervised validation should be a number of sequence and each number should be in [0,1)')

        if 0.5 not in self.parameters.missing_value_ratio:
            self.parameters.missing_value_ratio = self.parameters.missing_value_ratio.append(
                0.5)
            print(
                'Warning: Noise ratio should include 0.5 (for estimating a benchmark). Added')
    ...
    def validate_data_from_data_matrix(self, data_matrix,
        ...
        results = np.zeros((len(self.parameters.noise_ratio),
                            len(self.parameters.missing_value_ratio)))
        ...
        for i in range(self.parameters.n_trial):
            ...
            for j in range(len(self.parameters.noise_ratio)):
                noise_ratio = self.parameters.noise_ratio[j]
                cur_matrix = noise * noise_ratio + \
                    data_matrix_nparray * (1 - noise_ratio)
                for k in range(len(self.parameters.missing_value_ratio)):
                    print("k: " + str(k) + "/" +
                          str(len(self.parameters.missing_value_ratio)))
                    missing_value_ratio = self.parameters.missing_value_ratio[k]
                    missing_value_index = []
                    cur_matrix_missing = np.copy(cur_matrix)
                    n_missing_value_each_col = np.round(
                        missing_value_ratio * cur_matrix.shape[0]).astype(int)
                    start = time.time()
                    for colidx in range(cur_matrix.shape[1]):
                        cur_misi = np.random.choice(
                            cur_matrix.shape[0], n_missing_value_each_col, replace=False)
                        cur_matrix_missing[cur_misi, colidx] = np.nan
                        missing_value_index.append(cur_misi)
                    end = time.time()
                    ...
                    for colidx in range(cur_matrix.shape[1]):
                        correct_values.extend(
                            cur_matrix[missing_value_index[colidx], colidx].tolist())
                        imputed_values.extend(
                            imputed_cur_matrix[missing_value_index[colidx], colidx].tolist())

                    imputed_values = np.array(imputed_values) + 1e-6
                    correct_values = np.array(correct_values) + 1e-6
                    if j == 0 and k == 0:
                        np.savetxt('imputed_values_' + str(j) + '_' +
                                   str(k) + '.csv', imputed_values, delimiter=',')
                        np.savetxt('correct_values_' + str(j) + '_' +
                                   str(k) + '.csv', correct_values, delimiter=',')
                    cur_results[j, k] = np.mean(np.abs(imputed_values - correct_values + 0.0) / (
                        np.abs(imputed_values + 0.0) + np.abs(correct_values + 0.0))) * 100
                    end = time.time()
                    print("Calculating Error:" + str(end - start))
        ...
        results = pd.DataFrame(
            results,
            index=list(self.parameters.noise_ratio),
            columns=self.parameters.missing_value_ratio)
    ...
    def do_impute(self, matrix_to_impute):
        ...
        if self.parameters.impute_mode == parameter_set.constants.v_unsupervised_parameters_impute_mode_randomforest:
            imputed_cur_matrix = np.transpose(
                self.rfimpute.miss_forest_imputation(np.transpose(matrix_to_impute)))
        elif self.parameters.impute_mode == parameter_set.constants.v_unsupervised_parameters_impute_mode_knn:
            imputer = KNNImputer(n_neighbors=2,row_max_missing=1, col_max_missing=1)
            imputed_cur_matrix = np.transpose(
